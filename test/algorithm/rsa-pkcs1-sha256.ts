...
describe('rsa-v1_5-sha256', () => {
    ...
    describe('signing', () => {
        it('signs a payload', async () => {
            const signer = createSigner(rsaKeyPair.privateKey, 'rsa-v1_5-sha256');
    ...
    describe('verifying', () => {
        it('verifies a signature', async () => {
            const verifier = createVerifier(rsaKeyPair.publicKey, 'rsa-v1_5-sha256');
            const data = Buffer.from('some random data');
            const sig = createSign('sha256').update(data).sign({ key: rsaKeyPair.privateKey, padding: RSA_PKCS1_PADDING });
